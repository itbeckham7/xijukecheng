...
$thirdCsUrl = '0';
$courseID3 = '';
$firstCsImg = 'assets/images/sandapian/new_course/festival.png';
$secondCsImg = 'assets/images/sandapian/new_course/food.png';
$thirdCsImg = 'assets/images/sandapian/new_course/places.png';

if (isset($nccsSet[0])) {
    $firstCsUrl = base_url('middle/ncoursewares/view') . '/' . $nccsSet[0]->childcourse_id;
    $firstCsImg = $nccsSet[0]->childcourse_photo;
    $firstCsFree = $nccsSet[0]->childcourse_isfree;
    $courseID1 = $nccsSet[0]->childcourse_id;
}
if (isset($nccsSet[1])) {
    $secondCsUrl = base_url('middle/ncoursewares/view') . '/' . $nccsSet[1]->childcourse_id;
    $secondCsImg = $nccsSet[1]->childcourse_photo;
    $secondCsFree = $nccsSet[1]->childcourse_isfree;
    $courseID2 = $nccsSet[1]->childcourse_id;
}
if (isset($nccsSet[2])) {
    $thirdCsUrl = base_url('middle/ncoursewares/view') . '/' . $nccsSet[2]->childcourse_id;
    $thirdCsImg = $nccsSet[2]->childcourse_photo;
    $thirdCsFree = $nccsSet[2]->childcourse_isfree;
    $courseID3 = $nccsSet[2]->childcourse_id;
}
?>
<a href="<?= $firstCsUrl; ?>"
   class="btn nchildcourse_list"
   id="sh_firstcs_btn"
   data-cs-id="<?= $courseID1 ?>"
   free_course="<?= $firstCsFree; ?>"
   style="background:url(<?= base_url() . $firstCsImg; ?>) no-repeat;background-size: 100% 100%">
</a>
<a href="<?= $secondCsUrl; ?>"
   class="btn nchildcourse_list"
   id="sh_secondcs_btn"
   data-cs-id="<?= $courseID2 ?>"
   free_course="<?= $secondCsFree; ?>"
   style="background:url(<?= base_url() . $secondCsImg; ?>) no-repeat;background-size: 100% 100%">
</a>
<a href="<?= $thirdCsUrl; ?>"
   class="btn nchildcourse_list"
   id="sh_thirdcs_btn"
   data-cs-id="<?= $courseID3 ?>"
   free_course="<?= $thirdCsFree; ?>"
...
<script>
    ...
    var cwPermissions = JSON.parse('<?= json_encode($cws_permissions); ?>');

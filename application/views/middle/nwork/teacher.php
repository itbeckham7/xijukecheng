...
<style>
    ...
    .member_item{ font-family: memberFont;}
</style>
...
<!---------------------------Top Menu Area-------------------------------->
<a href="<?= base_url('middle/nchildcourse/index');?>"
   class="btn" id="sh_backhome_btn"
   style="background:url(<?= $imageAbsDir.'base/backhome.png';?>) no-repeat;background-size: 100% 100%;">
</a>
<?php if($this->session->userdata("loggedin") != FALSE){?>
    <a href="<?= base_url().'middle/users/profile/'.$loged_In_user_id;?>" class="btn" id="sh_profile_btn"  style="background:url(<?= $imageAbsDir.'base/profile.png';?>) no-repeat;background-size: 100% 100%;"> </a>
...
<style>
    ...
    #prevPage_Btn
    {
        ...
        left: 44%;
        top: 99%;
        background: url(../../../assets/images/sandapian/base/prev_btn.png);
        background-repeat: no-repeat;
        background-size: 100% 100%;
    }
    #prevPage_Btn:hover
    {
        background: url(../../../assets/images/sandapian/base/prev_btn_hover.png);
        background-repeat: no-repeat;
        background-size: 100% 100%;
    }
    #nextPage_Btn
    {
        ...
        left: 53%;
        top: 99%;
        background: url(../../../assets/images/sandapian/base/next_btn.png);
        background-repeat: no-repeat;
        background-size: 100% 100%;
    }
    #nextPage_Btn:hover
    {
        background: url(../../../assets/images/sandapian/base/next_btn_hover.png);
        background-repeat: no-repeat;
        background-size: 100% 100%;
    }
    #work_list_prev_btn{
    ...
    #work_list_prev_btn:active
    {
        outline: none !important;
    }
    #work_list_prev_btn:hover
    {
        background: url(../../../assets/images/sandapian/base/prev_btn_hover.png);
        background-repeat: no-repeat;
        background-size: 100% 100%;
    ...
    #work_list_next_btn:active
    {
        outline: none !important;
    }
    #work_list_next_btn:hover
    {
        background: url(../../../assets/images/sandapian/base/next_btn_hover.png);
        background-repeat: no-repeat;
        background-size: 100% 100%;
...
<div class="edit_content_area" style="display: none">
    ...
    <div id="nwork_pageNavPosition"></div>
</div>
<!------------------Student content list---------------------------->
<!------------------Search controls---------------------->
...
<script type="text/javascript">
    ...
    function Pager(tableName, itemsPerPage) {
        ...
        this.showPageNav = function(pagerName, positionId) {
            ...
            var pagerHtml = '<button class = "btn btn blue" id="work_list_prev_btn" onclick="' + pagerName + '.prev();" class="pg-normal">'+prevstr+ '</button>  ';
            for (var page = 1; page <= this.pages; page++)
                pagerHtml += '<button hidden id="pg' + page + '" class="pg-normal" onclick="' + pagerName + '.showPage(' + page + ');">' + page + '</button>  ';
            pagerHtml += '<button  class = "btn btn blue" id="work_list_next_btn" onclick="'+pagerName+'.next();" class="pg-normal">'+nextstr+'</button>';
...
<script>
    ...
    function clickStudent()///ajax_function for teacher let to edit student and itself information
    {
        $.ajax({
           type:"post",
           url:baseURL+'middle/nwork/getContents',
           dataType:"json",
           data:{userid:curUserIdForEdit,content_slug:curTab},
           success:function(res){
                if(res.status==='success'){
                    $('#edit_list_items').html(res.data);
                    pager = new Pager('edit_list_items', showedItems);
                    pager.init();
                    pager.showPageNav('pager', 'nwork_pageNavPosition');
                    pager.showPage(1);
    ...
    function changeContentEval(userid,contentId,hegeVal){
        $.ajax({
            type:"post",
            url:baseURL+'middle/nwork/changedContentEval',
            dataType:"json",
    ...
    function content_delete_ajax(contentId){
        jQuery.ajax({
            url:baseURL+"middle/nwork/delete_content",
            type:"post",
            dataType: "json",
    ...
    function showNContent(self) {
        ...
        if(curTab!='note')
        {
            ...
            jQuery.ajax({
                url:baseURL+"middle/nwork/get_NoteDetails",
                type:"post",
                dataType: "json",

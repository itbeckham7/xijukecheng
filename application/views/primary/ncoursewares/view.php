                                ...
                                ?>
                                    <li>
                                        <a href="#" class="new_courseware_item" onclick="changeVideo(this)"
                                           ...
                                           data-url="<?= $dataUrl;?>"
                                           data-is-free = "<?= ($ncws->nfree != 1 && !in_array($cur_cs_id,$cws_permissions))?'no':'yes';  ?>"
...
<?php if($this->session->userdata("loggedin") != FALSE){?>
    ...
    <?php }?>
    <?php if($user_type=='1'){?>
        <a href="<?= base_url('primary/nwork/teacher').'/'.$loged_In_user_id?>"
           class="btn" id="workview_btn"
           style="background:url(<?= $imageAbsDir.'workview/workview.png';?>) no-repeat;background-size: 100% 100%;">
        </a>
    <?php }?>
...
<script>
    ...
    function changeVideo(self){
        ...
        var is_free = self.getAttribute('data-is-free');
    ...
    $('#sh_backhome_btn').click(function(){
        ...
        {
            $(this).attr('href',baseURL+'primary/nchildcourse/index');
        }else{
            $(this).attr('href','#');
            location.reload();
        }
...
<script src="<?= base_url('assets/js/custom/primary/ncourseware.js')?>"></script>

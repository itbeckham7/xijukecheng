...
class Work extends CI_Controller {
	...
	public function view($content_id)////$id is content id
    {
        ///we have to increase view num to contents.
            ...
            if ($content_type_id == '1') {///script work
                ...
                $this->data["subview"] = "primary/work/content";
                ...
                $this->data["subview"] = "primary/work/content_shooting";
            }else {///dubbing work

                $this->data['wavPath'] = $contentInfo->file_name;
                $this->data["bgPath"] =  $contentInfo->bg_path;
                $this->data["subview"] = "primary/work/content_dubbing";
            }
	...
	public function script($user_id){
        ...
        if($this->signin_m->loggedin()){
            ...
            $this->data["subview"] = "primary/work/view";
    ...
    public function student()///this function is for teacher of front end
    {
        if($this->signin_m->loggedin()) {
            ...
            $this->data["subview"] = "primary/work/student";
            $this->load->view('primary/_layout_main', $this->data);
        }else{
            redirect(base_url().'signin/index');
        }
...
//              $ret['data'] = $this->output_content($this->data['contents'],$usingStatus);
    ...
    public function output_content($contents,$usingStatus)
    {
        ...
        foreach ($contents as $content):
           ...
           if($usingStatus) {///current user is student
               ...
               $output .= '<a href="' . base_url('primary/work/view') .$content->content_id . '">';
    ...
    public function output_members($users,$content_type_id)
    {
        ...
        $output = '';
        $output .= '<tr>';
        $output .= '<td><a href="'.base_url('primary/').$studentURL.$user_id.'">'.$this->lang->line('Me');
        $output .=  '</a>';
        $output .=  '</td>';
        $output .= '</tr>';
        ...
        foreach ($users as $user):
            if($this->users_m->hasContents($user->user_id,$content_type_id)){
                ...
                $output .='<td><a href="'.base_url('primary/').$studentURL.$user->user_id.'">'.$user->fullname.'</a></td>';
                $output .='</tr>';
            }
        endforeach;
        return $output;
    ...
    public function output_members_custom($users,$content_type_id)
    {
        ...
        $output .=         '<a href="'.base_url('primary/').$studentURL.$user_id.'" class="member_item">'.'&nbsp&nbsp'.$this->lang->line('Me').'</a>';
        $output .=      '</div>';
        ...
        foreach ($users as $user):
            //if($this->users_m->hasContents($user->user_id,$content_type_id)){
                ...
                if($extraCounts==0){
                    ...
                    $output .=     '<div class="member_item_wrapper">';
                    $output .=         '<a href="'.base_url('primary/').$studentURL.$user->user_id.'" class="member_item" >'.($serialNo+1).'&nbsp'.$user->fullname.'</a>';
                    $output .=      '</div>';

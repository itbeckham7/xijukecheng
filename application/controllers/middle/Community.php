...
class Community extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->signin_m->loggedin()) {
            ...
            $this->data["subview"] = "middle/community/index";
    ...
    public function view($content_id)//this function show comment page
    {
        ...
        if ($this->signin_m->loggedin()) {
            ...
            if ($content_type_id == '1') {///script work
                ...
                $this->data["subview"] = "middle/community/content";
                ...
                $this->data["subview"] = "middle/community/content_shooting";

            } else {

                $this->data['wavPath'] = $contentInfo->file_name;
                $this->data["bgPath"] = $contentInfo->bg_path;
                $this->data["subview"] = "middle/community/content_dubbing";
            }
    ...
    public function output_content($contents)
    {
        ...
        foreach ($contents as $content):
            $output .= '<tr>';
            $output .= '<td>';
            $output .= '<a href = "' . base_url('middle/') . 'community/view/' . $content->content_id . '">';
            $output .= '【' . $content->content_type_name . '】' . $content->content_title;
            $output .= '</a></td>';
            $output .= '<td><a href = "';
            $output .= base_url('middle/') . 'users/profile/' . $content->content_user_id . '">';
    ...
    public function output_custom_content($contents)
    {
        ...
        foreach ($contents as $content):
            ...
            $tempObj->content_url = base_url('middle/') . 'community/view/' . $content->content_id;
            $tempObj->profile_url = base_url('middle/') . 'users/profile/' . $content->content_user_id;
            $tempObj->author = $content->fullname;
            $tempObj->school = $content->school_name;
            $tempObj->view_num = $content->view_num;
            $tempObj->share_time = $content->share_time;
            $tempObj->share_time_str = $ago;
    ...
    public function output_comments($comments)
    {
        ...
        foreach ($comments as $comment):
            ...
            $output .= '<p style="font-weight: bold">' . $comment->fullname

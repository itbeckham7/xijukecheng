    ...
    public function profile($user_id)
    {
        ...
        if ($this->signin_m->loggedin()) {///if user logged in the the site =>
            ...
            if ($loggedIn_user_id == $user_id) {
                if (false && $user_type == 1) {///*********************************current user is teacher
                    ...
                    $this->data["subview"] = "xiaoxueapp/users/teacher";
                    $this->load->view('xiaoxueapp/_layout_main', $this->data);
                } else if ($user_type == 2 || $user_type == 1) {///********************************************current user is student
                    ...
                    $this->data["subview"] = "xiaoxueapp/users/student";
                    $this->load->view('xiaoxueapp/_layout_main', $this->data);
                } else {
                    $studentInfo = $this->users_m->get_single_user($user_id);
                    $this->data['student'] = $studentInfo;
                    $this->data['paidLists'] = $this->payhistory_m->getItemsFromUser($user_id);
                    $this->data["subview"] = "xiaoxueapp/users/weixin";
                    $this->load->view('xiaoxueapp/_layout_main', $this->data);
                }
                ...
                $this->data["subview"] = "xiaoxueapp/users/visitor";
                $this->load->view('xiaoxueapp/_layout_main', $this->data);
            }
        } else {//*****************************************************current user is visistor
            $userInfo = $this->users_m->get_single_user($user_id);
